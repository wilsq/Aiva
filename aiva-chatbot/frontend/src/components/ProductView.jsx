...
function ProductView( { product, onBack } ) {
    ...
    return (
        <div className="tuotenakyma">
            <button onClick={onBack} className="takaisinNappula">Takaisin</button>

            <h2>{product.name}</h2>
            <p><strong>Hinta:</strong> {product.price}€</p>
            <p><strong>Merkki:</strong> {product.brand}</p>
            <p><strong>Malli:</strong> {product.model}</p>
            <p><strong>Koko:</strong> {product.sizeInches}</p>
